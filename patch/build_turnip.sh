...
if [[ "$variant" == *"a8xx"* ]]; then
	# A8xx Master: The most feature-complete source (Whitebelyash + DiskDVD FSR + A8xx)
	mesasrc="https://github.com/DiskDVD/mesa-tu8.git"
	mesabranch="A8XX"
	srcfolder="mesa-a8xx-master"
	desc_extra="DiskDVD A8XX (FSR + Master Tuning)"
else
	# A6xx-A7xx Nightly: The stable clean foundation
...
prepare_workdir(){
	...
	if [ ! -d "$srcfolder" ]; then
		# No shallow clone to allow local patching and history checks
	...
	cd "$srcfolder"
	# Ensure SPIRV subprojects are present (Steven's stability fix)
	mkdir -p subprojects
	cd subprojects
...
apply_master_fixes(){
	...
	# 2. Registry Size Fix: Force 128 down to 96 (User Fix)
	...
	# 3. A7xx Preamble Fix (Steven's Stability Fix)
...
build_lib_for_android(){
	cd "$workdir/$srcfolder"
	echo "==== Compiling Mesa (LTO Disabled for stability) ===="
...
EOF
	...
	# LTO disabled to prevent build crashes
	...
	DESC="Turnip Nightly build: ${desc_extra}. KGSL build. Standard naming."
